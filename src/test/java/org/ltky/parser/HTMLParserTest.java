...
public class HTMLParserTest {
    ...
    private static final String KOTE_DEPARTMENT = "kote";

    private static final String KOTE_TEST_DATA = "<table cellspacing='0' border='0' width='100%' class='header-2-args'>\n" +
            "<col align='left' /><col align='center' /><col align='right' />\n" +
            "  <tr>\n" +
            "    <td><span class='header-2-0-0'>BK70A0101 - Simulation, Laboratory Course</span></td><td></td><td></td>\n" +
            "  </tr>\n" +
            "</table>\n" +
            "</td>\n" +
            "</tr><tr>\n" +
            ...
            "</tr>\n" +
            "<tr>\n" +
            "<td>BK70A0101 - Simulation, Laboratory Course/L</td>\n" +
            "<td>Mikkola, Aki</td>\n" +
            "<td>Periodi 4</td>\n" +
            "<td>10-15</td>\n" +
            "<td>to</td>\n" +
            ...
            "</tr>\n" +
            "<tr>\n" +
            "<td>BK70A0101 - Simulation, Laboratory Course/H</td>\n" +
            "<td>Mikkola, Aki</td>\n" +
            "<td>Periodi 3-4</td>\n" +
            "<td>2-7, 10-15</td>\n" +
            "<td>pe</td>\n" +
    ...
    public void HTMLLanguageLabMultiGroupTest() throws Exception {
        ...
        assertEquals(3, list.stream()
                ...
                .size());
        assertEquals(11, list.stream()
                .filter(c -> c.teacher.equals("Rantonen, Pirjo"))
                .collect(Collectors.toList())
                .size());
    ...
    @Test
    public void KoteDataParsingTest() throws IOException {
        CourseHtmlParser courseHtmlParser = new CourseHtmlParser(KOTE_DEPARTMENT);
        List<Course> list = courseHtmlParser.parseHTMLData(KOTE_TEST_DATA);
        assertEquals("Simulation, Laboratory Course", list.get(0).courseName);
        assertEquals("BK70A0101", list.get(0).courseCode);
        assertEquals("Yo-talo LS 204*", list.get(0).classroom);
        assertEquals("10-12", list.get(0).timeOfDay);
        assertEquals("to", list.get(0).weekDay);
        assertEquals("10,11,12,13,14,15", list.get(0).weekNumber);
        assertEquals("kote", list.get(0).department);
    }
